...
public:
  ...
  DepthFollower(int argc, char **argv)
      : it(nh)
  {
    if(VERBOSE_LOG) std::cout << "DepthFollower()" << std::endl;
    ...
    if(VERBOSE_LOG) std::cout << "..DepthFollower() DONE" << std::endl;
  }
  ...
  void imageCallback(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    if(stuck)
    {
      ...
      {
        if(speed > 0) speed = -speed; // invert speed
        if (VERBOSE_LOG)
      ...
      {
        // spinning
        if(speed < 0) speed = -speed; // restore speed
        winner.y = -1;
        winner.x = -1;
      ...
      move_robot(height, width, winner.x, winner.y, speed, speed);
      return;
    }
    ...
    else
    {
      ...
      else 
      {
        ...
        move_robot(height, width, winner.x, winner.y, linear, speed);
